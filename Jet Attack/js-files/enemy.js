...
function firstSpawnedValues () {
    for (let i = 0; i < eA.length; i++) {
        if (eA[i].firstSpawned) {
            ...
            if (eA[i].type == 1) {
                eA[i].ySpeed = randomDec(1, 1.5);
                eA[i].xSpeed = 6;
                eA[i].hp = 2 * enemyHealthMult;
                eA[i].aC = .5;
                eA[i].mAC = 0;
            } else if (eA[i].type == 2) {
                eA[i].ySpeed = randomDec(1.5, 2);
                eA[i].xSpeed = 7;
                eA[i].hp = 5 * enemyHealthMult;
                eA[i].aC = 1;
                eA[i].mAC = 0;
            } else if (eA[i].type == 3) {
                eA[i].ySpeed = randomDec(2, 2.5);
                eA[i].xSpeed = 9;
                eA[i].hp = 4 * enemyHealthMult;
                eA[i].aC = 0;
                eA[i].mAC = 1.5;
            } else if (eA[i].type == 4) {
                eA[i].ySpeed = randomDec(1.5, 2);
                eA[i].xSpeed = 7.5;
                eA[i].hp = 7 * enemyHealthMult;
                eA[i].aC = 2;
                eA[i].mAC = 1;
            }
    ...
    for (let i = 0; i < eA.length; i++) {
        ...
        if (eA[i].hp <= 0) {
            ...
            if (eA[i].stage <= 10) {
                ...
                if (enemySpawnTimer.s <= 3.8) {
                    enemySpawnTimer.s += .05;
                }
                if (testEnemyType(1)) {
                    player.score += 1000
                    localStorage.money = Number(localStorage.money)+blueDrop;
                } else if (testEnemyType(2)) {
                    player.score += 5000
                    localStorage.money = Number(localStorage.money)+purpleDrop;
                } else if (testEnemyType(3)) {
                    player.score += 4000
                    localStorage.money = Number(localStorage.money)+yellowDrop;
                } else {
                    player.score += 6000
                    localStorage.money = Number(localStorage.money)+redDrop;
                }
                eA.splice(i, 1);
            }
        }
...
function enemyHealthMultFunction () {
    if (enemiesKilled <= 50) {
        enemyHealthMult = 1;
        ...
        enemyHealthMult = 1.5;
    } else if (enemiesKilled <= 200) {
        enemyHealthMult = 2;
    } else if (enemiesKilled <= 300) {
        enemyHealthMult = 2.25;
    } else if (enemiesKilled > 300) {
        enemyHealthMult = 2.5;
    }
}
