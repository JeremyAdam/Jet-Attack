...
function gameEnds () {
    ...
    if (gameEnded) {
        ...
        if (eA.length == 0 && eM.length == 0 && eB.length == 0 && pM.length == 0 && pB.length == 0) {
            let temp = randomInt(1, 12);
            if (temp <= 5) {
                localStorage.money = Number(localStorage.money)+smallDrop;
            } else if (temp <= 8) {
                localStorage.money = Number(localStorage.money)+normalDrop;
            } else if (temp <= 10) {
                localStorage.money = Number(localStorage.money)+mediumDrop;
            } else {
                localStorage.money = Number(localStorage.money)+bigDrop;
            }
            gameEnded = false;
            showstart = true;
        }
